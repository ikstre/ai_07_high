...
from ui_steps import TARGET_CHANNEL_OPTIONS, SHOT_TYPE_OPTIONS, AD_TONE_OPTIONS
...
# ── Fix 2: shot_type 배선 ──────────────────────────────────────────────────
def test_shot_type_options_match_templates():
    """UI 구도 옵션(빈값 제외)이 백엔드 템플릿과 정확히 일치 — 드리프트 차단."""
    ui_keys = {s for s in SHOT_TYPE_OPTIONS if s}
    assert ui_keys == set(_COMPOSITION_TEMPLATES), (
        f"UI/백엔드 구도 불일치: UI={ui_keys}, BE={set(_COMPOSITION_TEMPLATES)}"
    )
